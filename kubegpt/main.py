...
def chat_group():
    '''
    This tool uses OpenAI's ChatGPT to chat about Kubernetes. It is enhanced with a retriever storing k8s documentation to improve results.
    It uses your OpenAI API key from $OPENAI_API_KEY to access the model.
    '''

@chat_group.command()
@click.option('--prompt', required=True, help='Initial prompt to start the conversation.')
def chat(prompt):
    """
